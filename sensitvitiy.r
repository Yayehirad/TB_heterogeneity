...
#c, significant
e_c=data.frame(cbind(output_matrix_equi$c, output_matrix_equi$Equi_incidence))
...
e_c_gp
#b, significant
e_b=data.frame(cbind(output_matrix_equi$b, output_matrix_equi$Equi_incidence))
e_b_g=melt(e_b,id='X1')
e_b_gp<-ggplot(e_b_g, aes(x=X1, y=value)) + 
  geom_point(color="coral4")+
  #geom_smooth(method = 'lm',se=FALSE,col='darkred')+
  
  xlab("low-spreader proportion")+
  #theme(plot.margin=margin(t = 0, r = 0, b = 0, l = 0, unit = "pt"))+
  ylab("")
e_b_gp
#alpha, significant
